...
class Stopwatch(Frame):
    ...
    def initUI(self):
        ...
        self.h_time     = 0
        self.s_time     = 0
        height01 = int(0.28 * self.height)
        self.Frame01 = tk.Frame(width=self.width, height = height01)
        self.Frame01.grid_propagate(0)
        self.Frame01.grid(row=0, column=0)
        self.Frame02 = tk.Frame(width=self.width, height = self.height - height01)
        self.Frame02.grid_propagate(0)
        self.Frame02.grid(row=1, column=0)
        self.Disp_Stopwatch = tk.Label(self.Frame01, height=1, width=11, font=("Helvetica", int(self.width/8.42)), text="00:00:00.000")
        self.Disp_Stopwatch.grid(row = 0, column = 0, sticky = W)
        self.Labels1 = []
        for pos in range(0,11):
           self.Disp_Laptime = tk.Label(self.Frame02, height=1, width=int(self.width/(self.width/28)), font=("Helvetica", int(self.width/48)), text="")
           self.Disp_Laptime.grid(row = pos+1, column = 4,sticky = W)
           self.Labels1.append(self.Disp_Laptime)
        self.Button_Start = tk.Button(self.Frame02, text = "Start", bg = "green",fg = "white",width = int(self.width/44), height = int(self.height/80),font = int(self.height/18), command = self.Start, justify=CENTER)
        self.Button_Start.grid(row = 1, column = 0,columnspan = 2, rowspan = 4)
        self.Button_Stop = tk.Button(self.Frame02, text = "Stop",bg = "red", width = int(self.width/44), height = int(self.height/80),font = 18,command=self.Stop, justify=CENTER)
        self.Button_Stop.grid(row = 1, column = 2,columnspan = 2,  rowspan = 4)
        self.Button_Laptime = tk.Button(self.Frame02, text = "Laptime", fg = "black",bg = "light blue", width = int(self.width/44), height = int(self.height/100),font = 18,command=self.Laptime, justify=CENTER)
        self.Button_Laptime.grid(row = 5, column = 0,columnspan = 2,  rowspan = 3)
        self.Button_Exit = tk.Button(self.Frame02, text = "Exit", bg = "blue",fg = "white", width = int(self.width/44), height = int(self.height/100),font = 18,command=self.Exit, justify=CENTER)
        self.Button_Exit.grid(row = 5, column = 2,columnspan = 2,  rowspan = 3)
        self.Button_Hours = tk.Button(self.Frame02, text = "Hrs", bg = "yellow",fg = "black", width = int(self.width/160), height = int(self.height/160),font = 18,command=self.Hours, justify=CENTER,repeatdelay=1000, repeatinterval=250)
        self.Button_Hours.grid(row = 8, column = 0,  rowspan = 2)
        self.Button_Mins = tk.Button(self.Frame02, text = "Min", bg = "yellow",fg = "black", width = int(self.width/160), height = int(self.height/160),font = 18,command=self.Mins, justify=CENTER,repeatdelay=1000, repeatinterval=250)
        self.Button_Mins.grid(row = 8, column = 1,  rowspan = 2)
        self.Button_Secs = tk.Button(self.Frame02, text = "Sec", bg = "yellow",fg = "black", width = int(self.width/160), height = int(self.height/160),font = 18,command=self.Secs, justify=CENTER,repeatdelay=1000, repeatinterval=250)
        self.Button_Secs.grid(row = 8, column = 2,  rowspan = 2)
        self.Button_Clr = tk.Button(self.Frame02, text = "Clr", bg = "yellow",fg = "black", width = int(self.width/160), height = int(self.height/160),font = 18,command=self.Clear, justify=CENTER)
    ...
    def Laptime(self):
        if self.run == 1 and time.time() - self.l_time > .2:
            ...
            if self.num < 10:
                snum = "0" + snum
            ltext = snum + " - " + self.msg + "   " + str(tmsg) + str(p)[1:5]
            print (ltext)
            self.Labels1[self.pos - 1].config(text= ltext)
